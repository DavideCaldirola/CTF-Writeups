...
> **Pwned with StealthGuy**, **[Writeup by: StealthGuy](https://stealthguy.net/kaboomthm/kaboom/)**

![Image](https://github.com/user-attachments/assets/2db30a3b-7965-4ad4-8bd1-737d0fd3cd22)
...
The first thing we did was visit the website on port `80`. There was a video of a working pump. This webpage was populated using a script in the HTML that fetches data from `/api/state`:

![Image](https://github.com/user-attachments/assets/099392ea-c0c4-4e87-985c-33dba843e813)

Next, we visited port `8080` and found an **OpenPLC v3** login page. From previous experience with OpenPLC, we knew that the default credentials are `openplc:openplc`, so we tried them — and they worked.

Then we checked out port `1880`, which showed a **Node-RED** login page. However, we couldn't access it.

We then turned our attention to ports `502`, `102`, and `44818`, which are commonly used in the OT (**Operational Technology**) domain — specifically, **Modbus TCP**, **Siemens S7** (proprietary), and **Ethernet/IP** (Rockwell), respectively.

OpenPLC typically communicates via Modbus TCP because it's open and widely supported. While OpenPLC does support S7 and Ethernet/IP through additional libraries, Modbus is the easiest to interact with. So, we began inspecting the Modbus interface.

Modbus primarily uses the concept of **coils** — boolean variables (ON/OFF). To explode the pump, we had the idea to turn all the coils ON.

To do this, we connected to the PLC on port `502` using a Python library called `pymodbus`, which enables Modbus communication:
...
![Image](https://github.com/user-attachments/assets/db7ab41f-2864-4389-8a70-b8c59ecc0e1d)
...
![Image](https://github.com/user-attachments/assets/cb4f6c59-79c6-4734-860e-f360972dc744)

![Image](https://github.com/user-attachments/assets/66ce5cfc-1543-43e9-b45d-1b1533e182c1)
...
![Image](https://github.com/user-attachments/assets/e888e036-fc25-44d0-a0c2-c74f74040ae2)

We can now navigate the **Ubuntu filesystem** and retrieve the flag located at `/home/ubuntu/flag.txt`:

![Image](https://github.com/user-attachments/assets/c250b9f2-57e0-438f-b1fc-432b6e23967f)
